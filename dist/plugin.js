...
exports.preview = "https://github.com/8gudbits/hfs-chat-plus/raw/main/preview.png"
...
exports.init = async (api) => {
  ...
  async function addMsg({ ctx, ts, method }) {
    ...
    while (max && chatDb.size() > max) chatDb.del(chatDb.firstKey())
  }
  ...
  return {
    async middleware(ctx) {
      if (
        ctx.path.startsWith(NOTIFICATIONS_URL) &&
        ctx.query.channel === CHANNEL &&
        !isAllowed(getCurrentUsername(ctx), "read")
      ) {
        ctx.status = 403
        return ctx.stop()
      }
      ...
      if (!Object.values(apis).includes(p)) return
